...
import { ArrowRight, DollarSign, Laptop, Code, Brush } from 'lucide-react';
import { recommendMacBook, budgetRanges } from '../data/macbookData';
...
const steps: QuizStep[] = [
  {
    ...
    type: 'single',
    options: [
      { value: 'budget', label: 'Budget-Friendly (Under $1,200)', icon: <DollarSign size={24} /> },
      { value: 'midRange', label: 'Mid-Range ($1,201-$2,000)', icon: <DollarSign size={24} /> },
      { value: 'premium', label: 'Premium ($2,001-$3,000)', icon: <DollarSign size={24} /> },
      { value: 'luxury', label: 'Luxury ($3,000+)', icon: <DollarSign size={24} /> }
    ]
  },
  {
    question: 'What will you primarily use the MacBook for?',
    type: 'multiple',
    options: [
      { value: 'students', label: 'School/Studies', icon: <Laptop size={24} /> },
      { value: 'developers', label: 'Programming', icon: <Code size={24} /> },
      { value: 'creators', label: 'Content Creation', icon: <Brush size={24} /> },
      { value: 'professional', label: 'Professional Work', icon: <Laptop size={24} /> }
    ]
  },
  {
    ...
    type: 'multiple',
    options: [
      { value: 'portable', label: 'Portability', icon: <Laptop size={24} /> },
      { value: 'power-users', label: 'Performance', icon: <Laptop size={24} /> },
      { value: 'video-editing', label: 'Large Screen', icon: <Laptop size={24} /> },
      { value: 'everyday', label: 'Battery Life', icon: <Laptop size={24} /> }
    ]
  }
];
  ...
  if (showResults) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h2 className="text-3xl font-bold mb-8 text-center">Your Perfect MacBook Matches</h2>
        <div className="grid gap-8 md:grid-cols-3">
          {recommendations.map(macbook => (
            <div key={macbook.id} className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col">
              <img src={macbook.imageUrl} alt={macbook.model} className="w-full h-48 object-cover" />
              <div className="p-6 flex-grow">
                <h3 className="text-xl font-bold mb-2">{macbook.model}</h3>
                <div className="space-y-2 text-sm text-gray-600">
                  <p>✓ {macbook.chip} Chip</p>
                  <p>✓ {macbook.display}</p>
                  <p>✓ {macbook.ram}</p>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {macbook.bestFor.map(feature => (
                    <span key={feature} className="px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs">
                      {feature}
                    </span>
                  ))}
                </div>
                <p className="mt-4 text-sm text-gray-600">{macbook.description}</p>
              </div>
              <div className="p-6 pt-0">
                <a
                  href={macbook.amazonUrl}
                  target="_blank"
